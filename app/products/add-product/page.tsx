...
export default function AgregarProductoPage() {
  ...
  // Formatea número con puntos cada 3 dígitos: 1220099 → "1.220.099"
  const fmtPrecio = (n: number) =>
    n === 0 ? "" : n.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".");

  // Quita puntos y devuelve número: "1.220.099" → 1220099
  ...
  const handlePrecioCompra = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    setPrecioCompraDisplay(pc ? pc.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".") : "");
    setPrecioVentaDisplay(pv ? pv.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".") : "");
